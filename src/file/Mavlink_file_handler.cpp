...
MavlinkFileHandler::MavlinkFileHandler(std::string folderPath)
{
    ...
    mavlink_altitude_fileStream = std::fstream(folderPath+"mavlink_altitude_t_.csv");
    mavlink_highres_imu_fileStream = std::fstream(folderPath+"mavlink_highres_imu_t.csv");
    mavlink_battery_status_fileStream = std::fstream(folderPath+"mavlink_battery_status_t.csv");
...
void MavlinkFileHandler::write_mavlink_altitude_file(mavlink_altitude_t altitude){
    ...
    if(fs::file_size(folderPath+"mavlink_altitude_t_.csv") == 0)
    {
...
{
    if(fs::file_size(folderPath+"mavlink_highres_imu_t.csv") == 0)
    {
...
void MavlinkFileHandler::write_mavlink_battery_status_file(mavlink_battery_status_t battery_status)
{
    if(fs::file_size(folderPath+"mavlink_highres_imu_t.csv") == 0)
    {
        mavlink_highres_imu_fileStream << "current_consumed" <<separator<< "energy_consumed" <<separator<< "temperature" <<separator<< "voltages" <<separator<< "current_battery" <<separator<< "id" <<separator<< "battery_function" <<separator<< "type" <<separator<< "battery_remaining" <<separator<< "time_remaining" <<separator<< "charge_state"  <<  std::endl ;
    }
